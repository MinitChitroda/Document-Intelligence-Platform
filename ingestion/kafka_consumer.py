...
def run_consumer():
    ...
    for message in consumer:
        ...
        source_type = "unknown"
        # Resolve raw_path supporting tenant subfolders
        ext = os.path.splitext(filename)[1] if filename else ""
        parts = file_hash.rsplit("_", 1)
        tenant_dir = parts[0] if len(parts) == 2 else ""
        raw_path = os.path.join("data", "raw", tenant_dir, f"{file_hash}{ext}")
        logger.info(f"DEBUG_CONSUMER: raw_path set to: {os.path.abspath(raw_path)} (exists: {os.path.exists(raw_path)})")
        if not os.path.exists(raw_path):
            raw_path = os.path.join("data", "raw", f"{file_hash}{ext}")
            logger.info(f"DEBUG_CONSUMER: fallback raw_path set to: {os.path.abspath(raw_path)} (exists: {os.path.exists(raw_path)})")
        ...
        if not existing:
            ...
            try:
                ...
                passed = evaluate_document(db, doc_id, raw_path, text_content, ocr_confidence or 0.0, tenant_id=tenant_id)
                db.commit()
